...
async def evaluate_patient_and_recommend(
    ...
    # 4. Invoke the Swarm Orchestrator to negotiate the bed placement
    from services.swarm_orchestrator import orchestrate_allocation
    return await orchestrate_allocation(db, patient.id, int(score), shadow_mode_enabled=shadow_mode_enabled)
